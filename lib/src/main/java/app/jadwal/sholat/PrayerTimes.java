...
public class PrayerTimes {
    ...
    PrayerTimes(long millis, double... prayerTimes) {
        ...
        this.millis = millis;
        if (prayerTimes[PrayersType.SUNRISE.getIndex()] == NEGATIVE_INFINITY
                || prayerTimes[PrayersType.MAGHRIB.getIndex()] == POSITIVE_INFINITY) {
    ...
    public Date getPrayTime(PrayersType payersType) {
        ...
        if (isUseSecond())
            return new Date(this.millis + (long) ceil(time * Constants.MINUTE_IN_HOUR * Constants.SECOND_IN_MINUTE)
    ...
    /**
     * Whether second is used, if not it will be zero and minute is rounded to the
     * top (ceil).
     */
